...
const headingsArray = ["h1","h2","h3","h4","h5"];
let nonAIDHeadings = ["h1","h2","h3"];
    ...
    src.on('end', (() => {
        ...
        mammoth.convertToHtml({path: target_path})
            .then((result) => {
                ...
                let listOfSplitDocs = splitDocument(result.value, appName, filename, isAID);
...
const splitDocument = (doc, appName, filename, isAid) => {
    let h1s = [];
    let h2s = [];
    const $ = cheerio.load(doc);
    $('h1').each((index, ele) => h1s.push($(ele)));
    $('h2').each((index, ele) => h2s.push($(ele)));
    let objArray = [];
    let obj = '';
    let heading = '';
    let condition = true;
    if (h1s.length > 0) {
        h1s.forEach(h1 => {
            if (obj) {
                objArray.push({
                    "Item Name": heading,
                    "Documentation with HTML": obj,
                    "Documentation": striptags(obj, [], ' '),
                    "metadata": {
            ...
            obj = '';
            heading = h1.text();
            if(isAid && heading.toLowerCase() === "objectives"){
                return;
            }
            h1.nextUntil('h1').each((index, el) => {
                if(isAid) {
                    condition = config.AID.includes($(el).text().trim().toLowerCase());
                }
                if (headingsArray.includes(el.name) && condition) {
                    if (obj) {
                        if((el.name === 'h4' || el.name === 'h5') && !isAid && obj.length < 500){
                            obj += $(el).html();
                        } else {
                            objArray.push({
                                "Item Name": heading,
    ...
    objArray.forEach(o => console.log("ITEM NAME :::::", o["Item Name"]));
    return objArray;
};
